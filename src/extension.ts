...
async function setupGitIntegration(): Promise<void> {
  try {
    ...
    if (!gitExtension) {
      console.log(
        '🛡️ Code Guardian: Git extension not found - using hooks only'
      );
      setupGitHooks();
      return;
    }
    ...
    if (!gitExtension.isActive) {
      console.log('🛡️ Code Guardian: Waiting for Git extension to activate...');
      await gitExtension.activate();
    }

    const git = gitExtension.exports?.getAPI(1);
    if (!git) {
      console.log('🛡️ Code Guardian: Git API not available - using hooks only');
      setupGitHooks();
      return;
    }

    console.log('🛡️ Code Guardian: Git integration successful!');
    ...
    console.error('🛡️ Code Guardian: Git integration failed:', error);
...
if git diff --cached --name-only | xargs grep -l -E "(//|#).*($BLOCKING_WORDS)" 2>/dev/null; then
    echo ""
    echo "🚫 Code Guardian: Cannot commit! BLOCKING comments found:"
    echo "================================================================"
...
echo "✅ Code Guardian: No blocking comments found - commit allowed!"
exit 0
`;
}
...
export function activate(context: vscode.ExtensionContext) {
  console.log('🛡️ Code Guardian is now active!');
  ...
  // Log para debug
  console.log('🛡️ Code Guardian: All components registered successfully');
}
...
export function deactivate() {
  ...
  console.log('🛡️ Code Guardian deactivated');
}
